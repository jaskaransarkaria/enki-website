...
test("Classes Index Page", async ({ page }) => {
  ...
  await page.locator("here").click()
  await page.waitForURL("http://localhost:4000/contact")
  await page.goBack()
  await page.waitForURL("http://localhost:4000/classes/beginners")

  await expect(page.locator(".class-pics > p")).toHaveText(`Ever wanted to learn to make your own jewellery? Looking for a unique
  ...
  await expect(page.locator(".tt-widget-fallback")).toBeHidden()
})
