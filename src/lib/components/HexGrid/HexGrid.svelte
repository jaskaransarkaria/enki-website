<script lang="ts">
	...
	import { getGridCols, calcShowGrid, calcRowNumber } from '$lib/utils/gridCalc';
	...
	let imgElemArr: HTMLImageElement[] = data.length > 0 ? [...createEmptyArray(data.length)] : [];
	...
	$: filteredData = data.filter(
		(base: Base): base is Category | Tag =>
			isTag(base) ||
			(isCategory(base) && base.NominalCode !== 'NOT_WEB' && base.NominalCode !== 'CLASSES')
	);
	...
	$: rowNumber =
		filteredData.length > gridColumnNumber
			...
			: 1;
	$: rowOffset = Math.floor(rowNumber % 2 === 0 ? rowNumber / 2 : (rowNumber - 1) / 2); // the even rows are 1 shorter
