<script lang="ts">
	...
	$: sortedCollatedArray = sortBy
		? collatedArray
				.slice()
				.sort((a, b) =>
					'SalePrice' in a ? (a.SalePrice > ('SalePrice' in b ? b.SalePrice : -1) ? -1 : 1) : -1
				)
...
<div class="container">
	{#if showSwitch}
		<div class="switch-container">
			<Switch bind:active={sortBy} msg={sortBy ? 'price (highest to lowest)' : 'alphabetically'} />
		</div>
	{/if}
	{#key sortBy}
				...
				!calcShowGrid(
					...
					variantCategories.length
						? variantCategories.length +
								...
								variantArr.filter((vars) => !variantCategoryIds.includes(vars.CategoryId)).length
						: productArr.length
				)
