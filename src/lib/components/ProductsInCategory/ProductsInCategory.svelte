<script lang="ts">
	...
	const refreshProductsArr = async (id: number): Promise<readonly Product[]> => {
		if (id) {
			const data = await refreshProducts(
				`${import.meta.env.VITE_SERVER_URL}/products-by-category?id=${id.toString()}`
			);
			productArr = data;
			return data;
		}
...
{#if categoryId && ((productArr.length && !productObj?.[categoryId]?.isEmpty) || variantCategories.length)}
	<ProductView {productArr} {showDetailedView} {variantCategories} {showSwitch} />
{/if}
