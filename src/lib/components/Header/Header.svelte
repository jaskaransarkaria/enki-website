<script lang="ts">
	...
	const grow = (width: number, x: number) => `width: ${width}px; transform: translateX(${x}px);`;
...
<div class="header">
	...
	{:else}
		<div class="left-container">
			<button class={selected.includes('shop') ? 'selected' : ''} on:click={() => goto('/shop')}>
				shop
			</button>
			<button
			...
			<button class={selected.includes('about') ? 'selected' : ''} on:click={() => goto('/about')}>
				about
			</button>
			<button
	...
	<div class="right-container">
		...
		{/if}
		<div class={isMobile ? 'mobile-basket-icon-container' : 'basket-icon-container'}>
			<img
				class="basket-icon"
			...
			>
				<g>
					...
					<text
						class={`${
							animateBasketNumber ? 'animate-basket-number-odd' : 'animate-basket-number-even'
						}`}
						x="55%"
						y="45%"
						text-anchor="middle"
