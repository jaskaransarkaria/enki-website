<script lang="ts">
  ...
  const removeItemFromBasket = (currentQuantity: number, list: any, listIndx: number): BasketProduct[] => currentQuantity === 1 ? [
  ...
  const updateBasket = (id: string, list: any, updateType: string): BasketProduct[] => {
...
<button on:click={() => basket.set(updateBasket(productId, $basket, "incrementQuantity"))}>
  Add to Basket
</button>
{#if $basket.findIndex(obj => obj.id === productId) >= 0}
  <button on:click={() => basket.set(updateBasket(productId, $basket, "decrementQuantity"))}>
    Remove from Basket
  </button>
{/if}